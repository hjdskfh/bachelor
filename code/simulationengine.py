...
class SimulationEngine:
    ...
    def delay_line_interferometer(self, power_dampened, t, peak_wavelength, value):
        ...
        for i in range(0, len(value), self.config.batchsize):
            ...
            power_1, power_2, t_upsampled = self.simulation_helper.DLI(flattened_power_batch_resampled, dt_new, tau, delta_L, f_0,  n_eff)
            # plt.plot(power_1[:len(t)*100], label = 'after DLI 1')
            # plt.plot(power_2[:len(t)*100], label = 'after DLI 2')
            # plt.legend()
            # plt.show()
            ...
            # Use interpolation to compute signal values at new time points
            signal_downsampled = np.interp(t_original_all_sym, t_upsampled, power_1)
